...
function build_libsodium() {
   ...
   gcc-14 libsodium.c \
    ...
    if [ -e "${libsodium_lib}/libsodium.so" ]; then
        cp "${libsodium_lib}/libsodium.so" .
    elif [ -e "${libsodium_lib}/libsodium.dylib" ]; then
        cp "${libsodium_lib}/libsodium.dylib" .
    fi
...
function build_rust_crypto() {
   ...
   clang -fsanitize=safe-stack -g crypto_rust.c \
       ...
       -static \
       -lpthread \
       -o rbuild_libsodiumust
}

function build_spectre() {
    echo "Building Spectre test program"
    gcc spectre.c -O0 -msse2 -static -lpthread -o spectre
}
