...
OPT_NETTLE_OBJS =  cpuid.$(OBJEXT) memxor-2.$(OBJEXT) aes128-encrypt-2.$(OBJEXT) aes128-decrypt-2.$(OBJEXT) aes192-encrypt-2.$(OBJEXT) aes192-decrypt-2.$(OBJEXT) aes256-encrypt-2.$(OBJEXT) aes256-decrypt-2.$(OBJEXT) cbc-aes128-encrypt-2.$(OBJEXT) cbc-aes192-encrypt-2.$(OBJEXT) cbc-aes256-encrypt-2.$(OBJEXT) ghash-set-key-2.$(OBJEXT) ghash-update-2.$(OBJEXT) salsa20-2core.$(OBJEXT) sha1-compress-2.$(OBJEXT) sha256-compress-2.$(OBJEXT)
OPT_HOGWEED_OBJS =  ecc-secp192r1-modp.$(OBJEXT) ecc-secp224r1-modp.$(OBJEXT) ecc-secp256r1-redc.$(OBJEXT) ecc-secp384r1-modp.$(OBJEXT) ecc-secp521r1-modp.$(OBJEXT) ecc-curve25519-modp.$(OBJEXT) ecc-curve448-modp.$(OBJEXT)

OPT_NETTLE_SOURCES = fat-x86_64.c 
...
INSTALL_HEADERS = $(HEADERS) version.h # mini-gmp.h
...
hogweed_OBJS = $(hogweed_SOURCES:.c=.$(OBJEXT)) \
	       $(OPT_HOGWEED_OBJS) # mini-gmp.$(OBJEXT)
...
$(LIBNETTLE_FORLINK): $(nettle_OBJS)
	$(LIBNETTLE_LINK) $(nettle_OBJS) -Wl,--version-script=libnettle.map -o $@ $(LIBNETTLE_LIBS)
...
$(LIBHOGWEED_FORLINK): $(hogweed_OBJS) $(LIBNETTLE_FORLINK)
	$(LIBHOGWEED_LINK) $(hogweed_OBJS) -Wl,--version-script=libhogweed.map -o $@ $(LIBHOGWEED_LIBS)
	-mkdir .lib 2>/dev/null
	(cd .lib \
