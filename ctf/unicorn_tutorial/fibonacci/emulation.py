from unicorn import *  # load basic unicorn constants
from unicorn.x86_const import *  # load x86-specific constants

import struct

BASE = 0x400000
STACK_ADDR = 0x0
STACK_SIZE = 1024*1024

INSTR_SKIP_LIST = [0x00000000004004EF, 0x00000000004004F6, 0x0000000000400502, 0x000000000040054F]

def read(name):
    with open(name, 'rb') as f:
        return f.read()
        
def u32(data):
    # takes 4-byte string and converts it to integer which represents data in little endian
    return struct.unpack("I", data)[0]
    
def p32(num):
    # takes a number and converts it to representing 4-byte string in little endian
    return struct.pack("I", num)

def hook_code(mu, address, size, user_data):
    # print(f"Tracing instruction at 0x{address:x}, instruction size = 0x{size:x}")
    if address in INSTR_SKIP_LIST:
        mu.reg_write(UC_X86_REG_RIP, address+size)
    elif address == 0x400560:
        c = mu.reg_read(UC_X86_REG_RDI)
        print(chr(c))
        mu.reg_write(UC_X86_REG_RIP, address+size)

mu = Uc(UC_ARCH_X86, UC_MODE_64)

mu.mem_map(BASE, 1024*1024)
mu.mem_map(STACK_ADDR, STACK_SIZE)

mu.mem_write(BASE, read("./fibonacci"))
mu.reg_write(UC_X86_REG_RSP, STACK_ADDR + STACK_SIZE - 1)

mu.hook_add(UC_HOOK_CODE, hook_code)

mu.emu_start(0x00000000004004E0, 0x0000000000400575)