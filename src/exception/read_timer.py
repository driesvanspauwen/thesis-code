...
class Timer():
    ...
    def rdtscp(self, emulator: EmulatorInterface):
        if self.active:
            ...
            emulator.mu.reg_write(UC_X86_REG_RAX, self.cycles & 0xFFFFFFFF)
            emulator.mu.reg_write(UC_X86_REG_RDX, (self.cycles >> 32) & 0xFFFFFFFF)
            self.reset()
        else:
            emulator.logger.log(f"\tRDTSC cycles: {self.cycles}")
            emulator.mu.reg_write(UC_X86_REG_RAX, 0)
            emulator.mu.reg_write(UC_X86_REG_RDX, 0)
            self.active = True
